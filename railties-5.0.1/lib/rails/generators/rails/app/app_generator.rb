...
module Rails
  ...
  module Generators
    ...
    class ARGVScrubber # :nodoc:
      ...
      def prepare!
        handle_version_request!(@argv.first)                                         # 当含有'--version', '-v' 打印版本并退出
        handle_invalid_command!(@argv.first, @argv) do                               # 命令有误时显示'--help'
      ...
      private
        ...
        def handle_invalid_command!(argument, argv)                                     # rails new的时候才会进行railsrc文件的处理
          if argument == "new"
            yield
          else
            ['--help'] + argv.drop(1)
        ...
        def handle_rails_rc!(argv)
          if argv.find { |arg| arg == '--no-rc' }                                       # 从命令去掉'--no-rc'选项, 返回
        ...
        def railsrc(argv)
          if (customrc = argv.index{ |x| x.include?("--rc=") })
            ...
            yield(argv.take(customrc) + argv.drop(customrc + 1), fname)                      # 传入去除rc选项的参数
          else
        ...
        def insert_railsrc_into_argv!(argv, railsrc)
          return argv unless File.exist?(railsrc)                                             # railsrc不存在直接返回
          extra_args = read_rc_file railsrc
          argv.take(1) + extra_args + argv.drop(1)                                            # 将railsrc中参数加入ARGV
        end
    end
  end
end
