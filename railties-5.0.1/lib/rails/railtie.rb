...
module Rails
  ...
  class Railtie
    ...
    class << self
      ...
      def inherited(base)                                             # 继承 hook
        unless base.abstract_railtie?
          subclasses << base
        end
      end

      def rake_tasks(&blk)                                             # blk传入app执行
        @rake_tasks ||= []
      ...
      def configure(&block)                                               # Railtie::Configurable
      ...
      protected
        ...
        def method_missing(name, *args, &block)                           # 类方法形式调用单例方法
    ...
    delegate :railtie_name, to: :class                                     # Railtie#railtie_name = Railtie.railtie_name

    def initialize #:nodoc:                                                # 抽象类不能被实例
      if self.class.abstract_railtie?
        raise "#{self.class.name} is abstract, you cannot instantiate it directly."
      end
    end
                                                                           # configure 即 instance_eval, 传入self执行
    ...
    def run_tasks_blocks(app) #:nodoc:
      extend Rake::DSL
      each_registered_block(:rake_tasks) { |block| instance_exec(app, &block) }         # 可使用DSL
    end

    private

    def each_registered_block(type, &block)                        # 祖先链从下往上执行
      klass = self.class
