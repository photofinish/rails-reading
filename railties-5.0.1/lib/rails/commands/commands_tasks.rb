...
EOT
    ...
    def run_command!(command)
      command = parse_command(command)                            # 只是处理version与help命令

      if COMMAND_WHITELIST.include?(command)                      # 白名单的命令使用send调用, 否则交给Rails::RakeProxy处理
        send(command)
      else
        run_rake_task(command)                                    # rake任务
      end
    end
    ...
    def server
      set_application_directory!                                            # 切换到项目根目录 以'config.cu'来判断
      require_command!("server")                                            # 加载对应文件'rails/commands/server'

      Rails::Server.new.tap do |server|
        # We need to require application after the server sets environment,
        # otherwise the --environment option given to the server won't propagate.
        require APP_PATH                                                    # 加载config/application.rb
